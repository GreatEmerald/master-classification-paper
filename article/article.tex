...
The overall results can be seen in table \ref{tab-accuracy}, whereas per-class results can be seen in figure \ref{fig-randomforest} and the relative statistics in \ref{sec-randomforest-relative}, figure \ref{fig-randomforest-relative}.
...
For an alternative visualisation showing the change of \gls{RMSE}, \gls{MAE} and \gls{ME} overall and per class, see \ref{sec-apu}, figure \ref{fig-apu}.
...
The most simple model we selected was the \glsentryfull{GLM}, also known as multivariate linear regression.
...
However, due to the limitations of the spectral mixture analysis methods \citep{somers_endmember_2011}, we could not use it with the features we selected (as listed in \ref{sec-inputdata}, table \ref{tab-inputdata}), as they do not form a linear mixture representing the land cover fractions.
Modern machine learning methods are advantageous for their ability to model complex nonlinear relationships between variables and thus make use of all of the features as shown in \ref{sec-varimp}, figure \ref{fig-varimp}.

\section{Multi-step model relative error comparison} \label{sec-randomforest-relative}
